#include <ros/ros.h>
#include "std_msgs/String.h"
#include <image_transport/image_transport.h>
#include <cv_bridge/cv_bridge.h>
#include <sensor_msgs/image_encodings.h>
#include <opencv2/imgproc/imgproc.hpp>
#include <opencv2/highgui/highgui.hpp>
#include <dynamic_reconfigure_test/hsvInts.h>

static const std::string OPENCV_WINDOW = "Image window";

/*
class ImageConverter
{

public:
  ImageConverter()
    : it_(nh_)
  {
    // Subscrive to input video feed and publish output video feed
    cv::namedWindow(OPENCV_WINDOW);
  }

  ~ImageConverter()
  {
    cv::destroyWindow(OPENCV_WINDOW);
  }

};
*/
void hsvcallback(const std_msgs::String::ConstPtr& msg)
{
	ROS_INFO("subscribe [%s]", msg->data.c_str());
}
void imageCb(const sensor_msgs::ImageConstPtr& msg)
{
    cv_bridge::CvImagePtr cv_ptr;
    try
    {
      cv_ptr = cv_bridge::toCvCopy(msg, sensor_msgs::image_encodings::BGR8);
    }
    catch (cv_bridge::Exception& e)
    {
      ROS_ERROR("cv_bridge exception: %s", e.what());
      return;
    }
	
	//dynamic_reconfigure_test::hsvInts srv;
	//if(&client.call(srv))
	//{
	//	ROS_INFO("S C");
	//}
	//else{
	//	ROS_ERROR("Fail");
	//}
    // Draw an example circle on the video stream
    if (cv_ptr->image.rows > 60 && cv_ptr->image.cols > 60)
      cv::circle(cv_ptr->image, cv::Point(150, 150), 100, CV_RGB(255,0,0));

    // Update GUI Window
    //cv::imshow(OPENCV_WINDOW, cv_ptr->image);
    cv::waitKey(3);
    
    // Output modified video stream
	//image_transport::Publisher image_pub_;
	//image_pub_ = it.advertise("/image_converter/output_video", 1);
	//image_pub_.publish(cv_ptr->toImageMsg());
}
int main(int argc, char** argv)
{
	ros::init(argc, argv, "image_converter");
	// ImageConverter ic;
	ros::NodeHandle n;
 	ros::NodeHandle nh_;
	
	ros::Subscriber sub;    

	image_transport::ImageTransport it(nh_);
	image_transport::Subscriber image_sub_;
	ROS_INFO("subscribe");
	

	cv::namedWindow(OPENCV_WINDOW);
	ros::ServiceClient client = n.serviceClient<dynamic_reconfigure_test::hsvInts>("hsv");
	image_sub_ = it.subscribe("/camera/rgb/image_rect_color", 1, imageCb);

ros::spin();
	
    	cv::destroyWindow(OPENCV_WINDOW);
  return 0;
}
